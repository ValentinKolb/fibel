...
- `search_docs({ query, locale? })` searches visible pages. An omitted locale uses `defaultLocale`.
...
Each endpoint closes over its own Fibel context. Navigation, locales, visible pages, custom-page context, and search results remain separate. This matches sites where `/docs` and `/ui` should be searched independently and avoids a shared registry or scope parameter.
